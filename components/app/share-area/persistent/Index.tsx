...
const Index = ({ roomId = null }: Props) => {
    ...
    const { currentRoom, isInitiator, setCurrentRoomIdRef, joinOrCreateRoom } = usePersistentShare();
    const { clientId, guestUser, authUser } = useAuthContext();

    const isReady = !!clientId && (guestUser || authUser);

    useEffect(() => {
        setCurrentRoomIdRef(roomId as string)
        if (isReady && roomId) {
    ...
    <div className="  min-h-[70vh] mt-4 rounded-sm p-2 sm:p-4">
       ...
       <div className=" md:col-span-1 w-full flex flex-col items-start justify-center text-sm">
            <div className="text-[.6rem] text-gray-500 dark:text-gray-400">ROOM ID </div>
            <div className="uppercase text-lime-500 font-semibold">{currentRoom.room_id}</div> 
       </div>
         <div className="md:col-span-1 w-full text-xs text-center text-gray-500 dark:text-gray-400">PERSISTENT SHARE AREA</div>
        <div className="md:col-span-1 w-full flex flex-col items-end justify-center text-sm"> 
            <div className="text-[.6rem] text-gray-500 dark:text-gray-400">ROLE </div>
            <div className="uppercase text-lime-500 font-semibold">{isInitiator ? "OWNER" : "MEMBER"}</div> 
        </div>
       </div>
       <TextContent />
    </div>) : (
